...
/*@ nullable_by_default @*/ 
class JMLValueToValueMap<K extends JMLType, V extends JMLType> extends JMLValueToValueRelation<K, V> {
    ...
    /*@  public normal_behavior
      ...
      @*/
    public JMLValueToValueMap(/*@ non_null @*/ K dv,
                                   /*@ non_null @*/ V rv)
    {
        super(dv, rv);
    }
    ...
    protected JMLValueToValueMap(JMLValueSet<JMLValueValuePair<K, JMLValueSet<V>>> ipset,
                                      JMLValueSet<K> dom,
                                      int sz)
    {
        super(ipset, dom, sz);
    ...
    /*@ public normal_behavior
      @    requires dv != null && rv != null;
      @    ensures \result != null
      @         && \result.equals(new JMLValueToValueMap<K, V>(dv, rv));
      @*/
    public static <SK extends JMLType, SR extends JMLType> /*@ pure @*/ /*@ non_null @*/
        JMLValueToValueMap<SK, SR>
        singletonMap(/*@ non_null @*/ SK dv,
                     /*@ non_null @*/ SR rv)
    {
        return new JMLValueToValueMap<SK, SR>(dv, rv);
    }
    ...
    public static <SK extends JMLType, SR extends JMLType> /*@ pure @*/ /*@ non_null @*/
        JMLValueToValueMap<SK, SR>
        singletonMap(/*@ non_null @*/ JMLValueValuePair<SK, SR> pair)
    {
        return new JMLValueToValueMap<SK, SR>(pair);
    }
    ...
    {
        JMLValueSet<V> img = elementImage(dv);
        if (img.int_size() == 1) {
            V res = img.choose();
            //@ assume res != null;
            return res;
    ...
    /*@ also
      @   public normal_behavior
      @     ensures \result instanceof JMLValueToValueMap<K, V>
      @          && ((JMLValueToValueMap<K, V>)\result)
      @                .theRelation.equals(this.theRelation);
      @*/
    //@ pure
    public /*@ non_null @*/ Object clone() 
    {
        return new JMLValueToValueMap<K, V>(imagePairSet_, domain_, size_);
    } //@ nowarn Post;
     ...
     * @see JMLValueToValueRelation#insert(K, V)
     */
    /*@  public normal_behavior
      @    requires dv != null && rv != null;
      ...
      @*/
    public /*@ non_null @*/
        JMLValueToValueMap<K, V> extend(/*@ non_null @*/ K dv,
                                       /*@ non_null @*/ V rv) 
    {
        JMLValueToValueMap<K, V> newMap = this.removeDomainElement(dv);
        newMap = newMap.disjointUnion(new JMLValueToValueMap<K, V>(dv, rv));
        return newMap;
    } //@ nowarn Exception;
    ...
    /*@  public normal_behavior
      ...
      @*/
    public /*@ non_null @*/
        JMLValueToValueMap<K, V> removeDomainElement(K dv)
    {
    ...
    /*@  public normal_behavior
      ...
      @    ensures (\forall JMLValueValuePair<D, V> pair; ;
    ...
    public /*@ non_null @*/ <D extends JMLType>
        JMLValueToValueMap<D, V>
        compose(/*@ non_null @*/ JMLValueToValueMap<D, K> othMap)
    {
    ...
    /*@  public normal_behavior
      @    requires othMap != null;
      @    ensures (\forall JMLObjectValuePair<D, V> pair; ;
      @                 \result.theRelation.has(pair) 
      @                    == (\exists K val;
      @                            othMap.elementImage(pair.key).has(val);
      @                            this.elementImage(val).has(pair.value) 
      ...
      @*/
    public /*@ non_null @*/ <D>
        JMLObjectToValueMap<D, V>
        compose(/*@ non_null @*/ JMLObjectToValueMap<D, K> othMap)
    {
        return super.compose(othMap).toFunction();
    }
    ...
    /*@  public normal_behavior
      @    requires dom != null;
      @    ensures (\forall JMLValueValuePair<K, V> pair; ; 
      @                 \result.theRelation.has(pair)
      @                    == 
      @                       dom.has(pair.key) 
      @                    && elementImage(pair.key).has(pair.value)
      ...
      @*/
    public /*@ non_null @*/ 
        JMLValueToValueMap<K, V> 
        restrictedTo(/*@ non_null @*/ JMLValueSet<K> dom) 
    {
    ...
    public /*@ non_null @*/ 
        JMLValueToValueMap<K, V>
        rangeRestrictedTo(/*@ non_null @*/ JMLValueSet<V> rng)
    {
    ...
    public /*@ non_null @*/ 
        JMLValueToValueMap<K, V>
        extendUnion(/*@ non_null @*/ JMLValueToValueMap<K, V> othMap) 
        throws IllegalStateException
    {
        JMLValueToValueMap<K, V> newMap 
            = this.restrictedTo(this.domain_.difference(othMap.domain_));
        if (newMap.size_ <= Integer.MAX_VALUE - othMap.size_) {
            return
                new JMLValueToValueMap<K, V>(newMap.imagePairSet_
    ...
    public /*@ non_null @*/ 
        JMLValueToValueMap<K, V>
        clashReplaceUnion(JMLValueToValueMap<K, V> othMap,
                          V errval)
        throws IllegalStateException
    {
        JMLValueSet<K> overlap = this.domain_.intersection(othMap.domain_);
        ...
        JMLValueToValueRelation<K, V> newRel;
        if (newMap.size_ <= Integer.MAX_VALUE - othMap.size_) {
            newRel = new JMLValueToValueRelation<K, V>(newMap.imagePairSet_
                                                      .union(othMap
                                                             .imagePairSet_),
                                                      newMap.domain_
                                                      .union(othMap.domain_),
    ...
    public /*@ non_null @*/ 
        JMLValueToValueMap<K, V> 
        disjointUnion(/*@ non_null @*/ JMLValueToValueMap<K, V> othMap) 
        throws JMLMapException, IllegalStateException
    {
        JMLValueSet<K> overlappingDom = domain_.intersection(othMap.domain_);
        if (overlappingDom.isEmpty()) {
            if (this.size_ <= Integer.MAX_VALUE - othMap.size_) {
                return new JMLValueToValueMap<K, V>(this.imagePairSet_
                                                   .union(othMap
                                                          .imagePairSet_),
                                                   this.domain_
                                                   .union(othMap.domain_),
