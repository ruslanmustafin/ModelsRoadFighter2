...
/*@ nullable_by_default @*/ 
public class JMLValueToValueRelationEnumerator<K extends JMLType, V extends JMLType>
    implements JMLEnumeration<JMLValueValuePair<K, V>>, JMLValueType {

    //@ public model JMLValueSet<JMLValueValuePair<K, V>> uniteratedPairs;      in objectState;
    ...
    //@ public invariant elementType == \type(JMLValueValuePair<K, V>);
    ...
    JMLValueToValueRelationEnumerator(/*@ non_null @*/
        ...
        //@ set owner = null;
        //@ set elementType = \type(JMLValueValuePair<K, V>);
        //@ set returnsNull = false;
        imagePairEnum = rel.imagePairs();
        if (imagePairEnum.hasMoreElements()) {
            JMLValueValuePair<K, JMLValueSet<V>> p = imagePairEnum.nextImagePair();
            key = p.key;
            //@ assume key != null;
            //@ assume p.value != null;
    ...
    protected
        JMLValueToValueRelationEnumerator(JMLValueToValueRelationImageEnumerator<K, V> ipEnum,
        ...
        //@ set returnsNull = false;
        imagePairEnum
            = (JMLValueToValueRelationImageEnumerator<K, V>)ipEnum.clone();
    ...
    /*@ also
      ...
      @*/
    public JMLValueValuePair<K, V> nextElement() throws JMLNoSuchElementException {
        if (imageEnum.hasMoreElements()) {
            V v = imageEnum.nextElement();
            //@ assume v != null && v instanceof _RangeType_;
            return new JMLValueValuePair<K, V>(key, v);
        } else if (imagePairEnum.hasMoreElements()) {
            JMLValueValuePair<K, JMLValueSet<V>> imagePair = imagePairEnum.nextElement();
    ...
    /*@ public normal_behavior
      ...
      @*/
    public /*@ non_null @*/ JMLValueValuePair<K, V> nextPair()
        throws JMLNoSuchElementException {
        JMLValueValuePair<K, V> aPair = nextElement();  //@ nowarn Pre;
        //@ assume aPair != null;
        return aPair;
    ...
    public /*@ non_null @*/ Object clone() {
        return new JMLValueToValueRelationEnumerator<K, V>(imagePairEnum,
                                                          imageEnum,
                                                          key);
    } //@ nowarn Post;
    ...
    public /*@ pure @*/ boolean equals(/*@ nullable @*/ Object oth) {
        if  (oth == null
            ...
            JMLValueToValueRelationEnumerator<K, V> js
                = (JMLValueToValueRelationEnumerator<K, V>) oth;
    ...
    protected /*@ spec_public pure @*/ /*@ non_null @*/
        JMLValueSet<JMLValueValuePair<K, V>> abstractValue()
    {
        JMLValueSet<JMLValueValuePair<K, V>> ret = new JMLValueSet<JMLValueValuePair<K, V>>();
        JMLValueToValueRelationEnumerator<K, V> enum2
            = (JMLValueToValueRelationEnumerator<K, V>) clone();
        while (enum2.hasMoreElements()) {
            //@ assume enum2.moreElements;
            JMLValueValuePair<K, V> aPair = enum2.nextPair();
