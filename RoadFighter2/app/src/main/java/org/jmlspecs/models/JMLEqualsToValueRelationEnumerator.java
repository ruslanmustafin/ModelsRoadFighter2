...
/*@ nullable_by_default @*/ 
public class JMLEqualsToValueRelationEnumerator<K, V extends JMLType>
    implements JMLEnumeration<JMLEqualsValuePair<K, V>>, JMLValueType {

    //@ public model JMLValueSet<JMLEqualsValuePair<K, V>> uniteratedPairs;      in objectState;
    ...
    //@ public invariant elementType == \type(JMLEqualsValuePair<K, V>);
    ...
    JMLEqualsToValueRelationEnumerator(/*@ non_null @*/
        ...
        //@ set owner = null;
        //@ set elementType = \type(JMLEqualsValuePair<K, V>);
        //@ set returnsNull = false;
        imagePairEnum = rel.imagePairs();
        if (imagePairEnum.hasMoreElements()) {
            JMLEqualsValuePair<K, JMLValueSet<V>> p = imagePairEnum.nextImagePair();
            key = p.key;
            //@ assume key != null;
            //@ assume p.value != null;
    ...
    protected
        JMLEqualsToValueRelationEnumerator(JMLEqualsToValueRelationImageEnumerator<K, V> ipEnum,
        ...
        //@ set returnsNull = false;
        imagePairEnum
            = (JMLEqualsToValueRelationImageEnumerator<K, V>)ipEnum.clone();
    ...
    public JMLEqualsValuePair<K, V> nextElement() throws JMLNoSuchElementException {
        if (imageEnum.hasMoreElements()) {
            ...
            return new JMLEqualsValuePair<K, V>(key, v);
        } else if (imagePairEnum.hasMoreElements()) {
            JMLEqualsValuePair<K, JMLValueSet<V>> imagePair = imagePairEnum.nextElement();
    ...
    /*@ public normal_behavior
      ...
      @*/
    public /*@ non_null @*/ JMLEqualsValuePair<K, V> nextPair()
        throws JMLNoSuchElementException {
        JMLEqualsValuePair<K, V> aPair = nextElement();  //@ nowarn Pre;
        //@ assume aPair != null;
        return aPair;
    ...
    public /*@ non_null @*/ Object clone() {
        return new JMLEqualsToValueRelationEnumerator<K, V>(imagePairEnum,
                                                          imageEnum,
                                                          key);
    } //@ nowarn Post;
    ...
    public /*@ pure @*/ boolean equals(/*@ nullable @*/ Object oth) {
        if  (oth == null
            ...
            JMLEqualsToValueRelationEnumerator<K, V> js
                = (JMLEqualsToValueRelationEnumerator<K, V>) oth;
    ...
    protected /*@ spec_public pure @*/ /*@ non_null @*/
        JMLValueSet<JMLEqualsValuePair<K, V>> abstractValue()
    {
        JMLValueSet<JMLEqualsValuePair<K, V>> ret = new JMLValueSet<JMLEqualsValuePair<K, V>>();
        JMLEqualsToValueRelationEnumerator<K, V> enum2
            = (JMLEqualsToValueRelationEnumerator<K, V>) clone();
        while (enum2.hasMoreElements()) {
            //@ assume enum2.moreElements;
            JMLEqualsValuePair<K, V> aPair = enum2.nextPair();
