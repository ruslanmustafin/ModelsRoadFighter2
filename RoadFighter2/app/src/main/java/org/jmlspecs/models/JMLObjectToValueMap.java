...
/*@ nullable_by_default @*/ 
class JMLObjectToValueMap<K, V extends JMLType> extends JMLObjectToValueRelation<K, V> {
    ...
    /*@  public normal_behavior
      ...
      @*/
    public JMLObjectToValueMap(/*@ non_null @*/ K dv,
                                   /*@ non_null @*/ V rv)
    {
        super(dv, rv);
    }
    ...
    protected JMLObjectToValueMap(JMLValueSet<JMLObjectValuePair<K, JMLValueSet<V>>> ipset,
                                      JMLObjectSet<K> dom,
                                      int sz)
    {
        super(ipset, dom, sz);
    ...
    /*@ public normal_behavior
      @    requires dv != null && rv != null;
      @    ensures \result != null
      @         && \result.equals(new JMLObjectToValueMap<K, V>(dv, rv));
      @*/
    public static <SK, SR extends JMLType> /*@ pure @*/ /*@ non_null @*/
        JMLObjectToValueMap<SK, SR>
        singletonMap(/*@ non_null @*/ SK dv,
                     /*@ non_null @*/ SR rv)
    {
        return new JMLObjectToValueMap<SK, SR>(dv, rv);
    }
    ...
    public static <SK, SR extends JMLType> /*@ pure @*/ /*@ non_null @*/
        JMLObjectToValueMap<SK, SR>
        singletonMap(/*@ non_null @*/ JMLObjectValuePair<SK, SR> pair)
    {
        return new JMLObjectToValueMap<SK, SR>(pair);
    }
    ...
    /*@ also
      @   public normal_behavior
      @     ensures \result instanceof JMLObjectToValueMap<K, V>
      @          && ((JMLObjectToValueMap<K, V>)\result)
      @                .theRelation.equals(this.theRelation);
      @*/
    //@ pure
    public /*@ non_null @*/ Object clone() 
    {
        return new JMLObjectToValueMap<K, V>(imagePairSet_, domain_, size_);
    } //@ nowarn Post;
     ...
     * @see JMLObjectToValueRelation#insert(K, V)
     */
    /*@  public normal_behavior
      @    requires dv != null && rv != null;
      ...
      @*/
    public /*@ non_null @*/
        JMLObjectToValueMap<K, V> extend(/*@ non_null @*/ K dv,
                                       /*@ non_null @*/ V rv) 
    {
        JMLObjectToValueMap<K, V> newMap = this.removeDomainElement(dv);
        newMap = newMap.disjointUnion(new JMLObjectToValueMap<K, V>(dv, rv));
        return newMap;
    } //@ nowarn Exception;
    ...
    /*@  public normal_behavior
      ...
      @*/
    public /*@ non_null @*/
        JMLObjectToValueMap<K, V> removeDomainElement(K dv)
    {
    ...
    /*@  public normal_behavior
      @    requires dom != null;
      @    ensures (\forall JMLObjectValuePair<K, V> pair; ; 
      @                 \result.theRelation.has(pair)
      @                    == 
      @                       dom.has(pair.key) 
      @                    && elementImage(pair.key).has(pair.value)
    ...
    public /*@ non_null @*/ 
        JMLObjectToValueMap<K, V>
        extendUnion(/*@ non_null @*/ JMLObjectToValueMap<K, V> othMap) 
        throws IllegalStateException
    {
        JMLObjectToValueMap<K, V> newMap 
            = this.restrictedTo(this.domain_.difference(othMap.domain_));
        if (newMap.size_ <= Integer.MAX_VALUE - othMap.size_) {
            return
                new JMLObjectToValueMap<K, V>(newMap.imagePairSet_
    ...
    public /*@ non_null @*/ 
        JMLObjectToValueMap<K, V>
        clashReplaceUnion(JMLObjectToValueMap<K, V> othMap,
                          V errval)
        throws IllegalStateException
    {
        ...
        JMLObjectToValueRelation<K, V> newRel;
        if (newMap.size_ <= Integer.MAX_VALUE - othMap.size_) {
            newRel = new JMLObjectToValueRelation<K, V>(newMap.imagePairSet_
                                                      .union(othMap
                                                             .imagePairSet_),
                                                      newMap.domain_
                                                      .union(othMap.domain_),
    ...
    public /*@ non_null @*/ 
        JMLObjectToValueMap<K, V> 
        disjointUnion(/*@ non_null @*/ JMLObjectToValueMap<K, V> othMap) 
        throws JMLMapException, IllegalStateException
    {
        ...
        if (overlappingDom.isEmpty()) {
            if (this.size_ <= Integer.MAX_VALUE - othMap.size_) {
                return new JMLObjectToValueMap<K, V>(this.imagePairSet_
                                                   .union(othMap
                                                          .imagePairSet_),
                                                   this.domain_
                                                   .union(othMap.domain_),
