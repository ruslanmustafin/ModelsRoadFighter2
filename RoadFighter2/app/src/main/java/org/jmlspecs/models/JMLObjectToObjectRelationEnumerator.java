...
/*@ nullable_by_default @*/ 
public class JMLObjectToObjectRelationEnumerator<K, V>
    implements JMLEnumeration<JMLObjectObjectPair<K, V>>, JMLValueType {

    //@ public model JMLValueSet<JMLObjectObjectPair<K, V>> uniteratedPairs;      in objectState;
    ...
    //@ public invariant elementType == \type(JMLObjectObjectPair<K, V>);
    ...
    JMLObjectToObjectRelationEnumerator(/*@ non_null @*/
        ...
        //@ set owner = null;
        //@ set elementType = \type(JMLObjectObjectPair<K, V>);
        //@ set returnsNull = false;
        imagePairEnum = rel.imagePairs();
        if (imagePairEnum.hasMoreElements()) {
            JMLObjectValuePair<K, JMLObjectSet<V>> p = imagePairEnum.nextImagePair();
            key = p.key;
            //@ assume key != null;
            //@ assume p.value != null;
    ...
    protected
        JMLObjectToObjectRelationEnumerator(JMLObjectToObjectRelationImageEnumerator<K, V> ipEnum,
        ...
        //@ set returnsNull = false;
        imagePairEnum
            = (JMLObjectToObjectRelationImageEnumerator<K, V>)ipEnum.clone();
    ...
    public JMLObjectObjectPair<K, V> nextElement() throws JMLNoSuchElementException {
        if (imageEnum.hasMoreElements()) {
            ...
            return new JMLObjectObjectPair<K, V>(key, v);
        } else if (imagePairEnum.hasMoreElements()) {
            JMLObjectValuePair<K, JMLObjectSet<V>> imagePair = imagePairEnum.nextElement();
    ...
    /*@ public normal_behavior
      ...
      @*/
    public /*@ non_null @*/ JMLObjectObjectPair<K, V> nextPair()
        throws JMLNoSuchElementException {
        JMLObjectObjectPair<K, V> aPair = nextElement();  //@ nowarn Pre;
        //@ assume aPair != null;
        return aPair;
    ...
    public /*@ non_null @*/ Object clone() {
        return new JMLObjectToObjectRelationEnumerator<K, V>(imagePairEnum,
                                                          imageEnum,
                                                          key);
    } //@ nowarn Post;
    ...
    public /*@ pure @*/ boolean equals(/*@ nullable @*/ Object oth) {
        if  (oth == null
            ...
            JMLObjectToObjectRelationEnumerator<K, V> js
                = (JMLObjectToObjectRelationEnumerator<K, V>) oth;
    ...
    protected /*@ spec_public pure @*/ /*@ non_null @*/
        JMLValueSet<JMLObjectObjectPair<K, V>> abstractValue()
    {
        JMLValueSet<JMLObjectObjectPair<K, V>> ret = new JMLValueSet<JMLObjectObjectPair<K, V>>();
        JMLObjectToObjectRelationEnumerator<K, V> enum2
            = (JMLObjectToObjectRelationEnumerator<K, V>) clone();
        while (enum2.hasMoreElements()) {
            //@ assume enum2.moreElements;
            JMLObjectObjectPair<K, V> aPair = enum2.nextPair();
