...
/*@ nullable_by_default @*/ 
public class JMLEqualsToObjectRelationEnumerator<K, V>
    implements JMLEnumeration<JMLEqualsObjectPair<K, V>>, JMLValueType {

    //@ public model JMLValueSet<JMLEqualsObjectPair<K, V>> uniteratedPairs;      in objectState;
     ...
     */
    protected /*@ non_null @*/ JMLObjectSetEnumerator<V> imageEnum;
    //@                                                   in uniteratedPairs;
    ...
    //@ public invariant elementType == \type(JMLEqualsObjectPair<K, V>);
    ...
    JMLEqualsToObjectRelationEnumerator(/*@ non_null @*/
        ...
        //@ set owner = null;
        //@ set elementType = \type(JMLEqualsObjectPair<K, V>);
        //@ set returnsNull = false;
        imagePairEnum = rel.imagePairs();
        if (imagePairEnum.hasMoreElements()) {
            JMLEqualsValuePair<K, JMLObjectSet<V>> p = imagePairEnum.nextImagePair();
            key = p.key;
            //@ assume key != null;
            //@ assume p.value != null;
            //@ assume p.value instanceof JMLObjectSet<V>;
            imageEnum = p.value.elements();
            //@ assume imageEnum.moreElements;
        } else {
            key = null;
            imageEnum = (new JMLObjectSet<V>()).elements();
            //@ assume !imageEnum.moreElements;
        }
    }
    ...
    protected
        JMLEqualsToObjectRelationEnumerator(JMLEqualsToObjectRelationImageEnumerator<K, V> ipEnum,
                                               JMLObjectSetEnumerator<V> iEnum,
                                               K k) {
        ...
        //@ set returnsNull = false;
        imagePairEnum
            = (JMLEqualsToObjectRelationImageEnumerator<K, V>)ipEnum.clone();
        imageEnum = (JMLObjectSetEnumerator<V>)iEnum.clone();
        key = k;
    ...
    /*@ also
      ...
      @*/
    public JMLEqualsObjectPair<K, V> nextElement() throws JMLNoSuchElementException {
        if (imageEnum.hasMoreElements()) {
            V v = imageEnum.nextElement();
            //@ assume v != null && v instanceof _RangeType_;
            return new JMLEqualsObjectPair<K, V>(key, v);
        } else if (imagePairEnum.hasMoreElements()) {
            JMLEqualsValuePair<K, JMLObjectSet<V>> imagePair = imagePairEnum.nextElement();
    ...
    /*@ public normal_behavior
      ...
      @*/
    public /*@ non_null @*/ JMLEqualsObjectPair<K, V> nextPair()
        throws JMLNoSuchElementException {
        JMLEqualsObjectPair<K, V> aPair = nextElement();  //@ nowarn Pre;
        //@ assume aPair != null;
        return aPair;
    ...
    public /*@ non_null @*/ Object clone() {
        return new JMLEqualsToObjectRelationEnumerator<K, V>(imagePairEnum,
                                                          imageEnum,
                                                          key);
    } //@ nowarn Post;
    ...
    public /*@ pure @*/ boolean equals(/*@ nullable @*/ Object oth) {
        if  (oth == null
            ...
            JMLEqualsToObjectRelationEnumerator<K, V> js
                = (JMLEqualsToObjectRelationEnumerator<K, V>) oth;
    ...
    protected /*@ spec_public pure @*/ /*@ non_null @*/
        JMLValueSet<JMLEqualsObjectPair<K, V>> abstractValue()
    {
        JMLValueSet<JMLEqualsObjectPair<K, V>> ret = new JMLValueSet<JMLEqualsObjectPair<K, V>>();
        JMLEqualsToObjectRelationEnumerator<K, V> enum2
            = (JMLEqualsToObjectRelationEnumerator<K, V>) clone();
        while (enum2.hasMoreElements()) {
            //@ assume enum2.moreElements;
            JMLEqualsObjectPair<K, V> aPair = enum2.nextPair();
