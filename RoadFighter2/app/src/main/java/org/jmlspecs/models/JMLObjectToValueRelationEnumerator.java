...
/*@ nullable_by_default @*/ 
public class JMLObjectToValueRelationEnumerator<K, V extends JMLType>
    implements JMLEnumeration<JMLObjectValuePair<K, V>>, JMLValueType {

    //@ public model JMLValueSet<JMLObjectValuePair<K, V>> uniteratedPairs;      in objectState;
     ...
     */
    protected /*@ non_null @*/ JMLValueSetEnumerator<V> imageEnum;
    //@                                                   in uniteratedPairs;
    ...
    //@ public invariant elementType == \type(JMLObjectValuePair<K, V>);
    ...
    JMLObjectToValueRelationEnumerator(/*@ non_null @*/
        ...
        //@ set owner = null;
        //@ set elementType = \type(JMLObjectValuePair<K, V>);
        //@ set returnsNull = false;
        imagePairEnum = rel.imagePairs();
        if (imagePairEnum.hasMoreElements()) {
            JMLObjectValuePair<K, JMLValueSet<V>> p = imagePairEnum.nextImagePair();
            key = p.key;
            //@ assume key != null;
            //@ assume p.value != null;
            //@ assume p.value instanceof JMLValueSet<V>;
            imageEnum = p.value.elements();
            //@ assume imageEnum.moreElements;
        } else {
            key = null;
            imageEnum = (new JMLValueSet<V>()).elements();
            //@ assume !imageEnum.moreElements;
        }
    }
    ...
    protected
        JMLObjectToValueRelationEnumerator(JMLObjectToValueRelationImageEnumerator<K, V> ipEnum,
                                               JMLValueSetEnumerator<V> iEnum,
                                               K k) {
        ...
        //@ set returnsNull = false;
        imagePairEnum
            = (JMLObjectToValueRelationImageEnumerator<K, V>)ipEnum.clone();
        imageEnum = (JMLValueSetEnumerator<V>)iEnum.clone();
        key = k;
    ...
    /*@ also
      ...
      @*/
    public JMLObjectValuePair<K, V> nextElement() throws JMLNoSuchElementException {
        if (imageEnum.hasMoreElements()) {
            V v = imageEnum.nextElement();
            //@ assume v != null && v instanceof _RangeType_;
            return new JMLObjectValuePair<K, V>(key, v);
        } else if (imagePairEnum.hasMoreElements()) {
            JMLObjectValuePair<K, JMLValueSet<V>> imagePair = imagePairEnum.nextElement();
    ...
    /*@ public normal_behavior
      ...
      @*/
    public /*@ non_null @*/ JMLObjectValuePair<K, V> nextPair()
        throws JMLNoSuchElementException {
        JMLObjectValuePair<K, V> aPair = nextElement();  //@ nowarn Pre;
        //@ assume aPair != null;
        return aPair;
    ...
    public /*@ non_null @*/ Object clone() {
        return new JMLObjectToValueRelationEnumerator<K, V>(imagePairEnum,
                                                          imageEnum,
                                                          key);
    } //@ nowarn Post;
    ...
    public /*@ pure @*/ boolean equals(/*@ nullable @*/ Object oth) {
        if  (oth == null
            ...
            JMLObjectToValueRelationEnumerator<K, V> js
                = (JMLObjectToValueRelationEnumerator<K, V>) oth;
    ...
    protected /*@ spec_public pure @*/ /*@ non_null @*/
        JMLValueSet<JMLObjectValuePair<K, V>> abstractValue()
    {
        JMLValueSet<JMLObjectValuePair<K, V>> ret = new JMLValueSet<JMLObjectValuePair<K, V>>();
        JMLObjectToValueRelationEnumerator<K, V> enum2
            = (JMLObjectToValueRelationEnumerator<K, V>) clone();
        while (enum2.hasMoreElements()) {
            //@ assume enum2.moreElements;
            JMLObjectValuePair<K, V> aPair = enum2.nextPair();
